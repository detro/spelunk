...
var (
	ErrSecretSourceBase64FailedDecoding = fmt.Errorf("failed to decode base64 secret")
)
...
func (s *SecretSourceBase64) DigUp(_ context.Context, coord types.SecretCoord) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w (%q): %w", ErrSecretSourceBase64FailedDecoding, coord.Location, err)
	}
